...
p1 <- ggplot(data = df, aes(x = ndvi_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#00ff00", "#ff8000", "#ff6666",
                          "#00cc00", "#00ff00"), alpha = 0.75)+
  xlab("NDVI")

p2 <- ggplot(data = df, aes(x = mbi_utm_cut))+
  geom_histogram(binwidth = 1, fill = c("#ff6666", "#ff8000", "#ff6666"), alpha = 0.75)+
  xlab("Mass balance index")

p3 <- ggplot(data = df, aes(x = dtr_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#ff6666", "#ff8000", "#00cc00",
                          "#00cc00", "#00ff00"), alpha = 0.75)+
  xlab("Distance to roads")

p4 <- ggplot(data = df, aes(x = dtw_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#ff6666", "#ff8000", "#ff8000",
                          "#00cc00", "#00ff00"), alpha = 0.75)+
  xlab("Distance to water")

p5 <- ggplot(data = df, aes(x = slope_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#ff8000", "#ff6666", "#ff6666",
                          "#ff8000", "#00cc00", "#00ff00", "#00ff00", "#00ff00", "#00ff00"), 
                 alpha = 0.75)+
  xlab("Slope")

p6 <- ggplot(data = df, aes(x = soils_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#00ff00", "#00ff00", "#ff6666",
                          "#00ff00", "#ff8000", "#ff8000", "#00ff00", "#00ff00"), 
                 alpha = 0.75)+
  xlab("Soil type")

p7 <- ggplot(data = df, aes(x = spi_utm_cut))+
  geom_histogram(binwidth = 1, 
                 fill = c("#ff6666", "#00ff00", "#00ff00",
                          "#00ff00", "#00ff00"), alpha = 0.75)+
